...
## 2026-06-11 (4차): 컨텍스트 허브 + 파일별 프로젝트 + 처리 내역 인박스 + 자동 이메일

### 네비게이션 개편 (컨텍스트를 강점으로)
1. **아이콘 네비** — 새 회의록(마이크) / 처리 내역(인박스) / 내 회의록(문서) / 컨텍스트(뇌) / 내 페이지(사람)
2. **컨텍스트 허브** — 프로젝트 + 내 컨텍스트가 "컨텍스트" 페이지 하위 탭으로 통합
   ("쓸수록 똑똑해지는" Summarying의 핵심을 전면에)

### 프로젝트 분류 지원
1. **파일 단위 프로젝트 선택** — 업로드 시 공통 옵션이 아닌 파일 카드마다 프로젝트 선택
   (하루에 여러 프로젝트 회의를 한꺼번에 돌리는 케이스), "+ 새 프로젝트" 인라인 생성 지원
2. **분류 대기 현황** — `GET /projects/legacy-names`: 자유 텍스트 이름만 적힌 회의록(초기 데이터)을
   이름별 건수로 집계, 컨텍스트 > 프로젝트 탭에 칩으로 표시. 클릭 시 내 회의록에서 해당 이름 검색
   → 회의록 열어 분류 → AI 메모리 재구축 흐름

### 처리 내역 인박스
- "처리 중" → **"처리 내역"**: 진행 중 작업 카드 + 최근 7일 완료 내역(메시지함 스타일)
- 무한 스크롤 (IntersectionObserver), 읽지 않음 강조, 클릭 시 상세
- `GET /summaries?days=7` 기간 필터 추가

### 완료 자동 이메일
- 회의록 생성 완료 시 서버가 자동으로 이메일 발송 (체크박스 제거)
- 이메일에 **"수정하러 가기"** 버튼 → `{APP_BASE_URL}/app#summary/{id}` 해시 딥링크로
  해당 회의록 상세 페이지 직행 (로그인 후 자동 이동)
- `APP_BASE_URL` 환경변수 (기본: https://meeting-bot.jonny.kim)

---

## 2026-06-11 (3차): 페이지 분리 + 리스트 중심 워크플로우 + 내 페이지

### 페이지 구조 개편 (한 화면 과밀 해소)
1. **업로드/처리중/결과를 별도 페이지로 분리**
   - 새 회의록 = 업로드 전용. 생성 시작하면 자동으로 "처리 중" 페이지로 이동
   - "처리 중" 네비 탭 — 작업이 있을 때만 노출, 활성 작업 수 배지
   - 완료된 회의록은 결과 화면 없이 "내 회의록" 리스트에 쌓임 (토스트로 알림 + 바로 보기)
   - 회의록 상세/편집은 리스트에서 클릭해 진입하는 별도 페이지 (아코디언 제거)
2. **내 회의록 리스트 개선**
   - 읽지 않음 표시: 열람 전 회의록에 티일 점 + 좌측 보더 (`summary_records.viewed_at`, 마이그레이션 d4f1a2b3c5e6)
   - 페이지네이션 (10건/페이지) + 정확한 총 건수 (서버 total 카운트)
3. **상세 페이지에서 프로젝트 분류 변경** — `PUT /summaries/{id}/project` (RAG 메타데이터 동기화 포함)
4. **AI 메모리 전체 재구축** — `POST /projects/{id}/rebuild-memory`: 프로젝트의 모든 회의록을
   시간순으로 재생해 메모리를 처음부터 다시 빌드 (과거 회의록을 나중에 분류한 경우용)
5. **내 페이지** — 누적 통계(총 회의록/STT 분/누적·이번달 비용), 모델별 사용 횟수·비용 표,
   회의록 생성 메타(시스템) 프롬프트 열람 (`GET /me/stats`, `GET /meta/system-prompt`)
