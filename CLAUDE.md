...
- 아이콘 네비 5개: 새 회의록(마이크) / 처리 내역(인박스, 최근 7일+무한스크롤) / 내 회의록(문서, 페이지네이션+읽지않음 점) / 컨텍스트(뇌, 프로젝트+내 컨텍스트 하위 탭) / 내 페이지(사람, 통계+메타 프롬프트)
- 상세·편집(detailView)은 리스트/내역/딥링크에서 진입. 생성 완료 시 결과를 바로 띄우지 않음 — unread로 쌓임 (`viewed_at` 마킹)
