...
- 아이콘 네비 4개: 새 회의록(마이크) / 내 회의록(문서, 페이지네이션+읽지않음 점·완료 미열람 배지) / 컨텍스트(뇌, 프로젝트+내 컨텍스트 하위 탭) / 내 페이지(사람, 통계+메타 프롬프트)
- **처리는 백그라운드** — 별도 "처리 중" 페이지 없음. 생성 직후 화면 이동 없이 **우측 하단 처리 도크**(`#procDock`, 모든 화면 위 fixed)에서 ambient하게 진행. 알약(접힘) ↔ 패널(펼침), 작업당 카드(진행률·취소·완료시 "보기"·실패시 "다시 시도"). 도크 로직은 `jobs.js`, 작업은 브라우저 메모리(XHR)라 탭 닫으면 중단(beforeunload 경고 유지)
- 피드백 3층: **토스트**(시작/완료/실패 순간) · **도크**(세션 진행 상황) · **내 회의록**(영속 기록 — 완료물이 unread로 쌓임 + 상단 "N개 처리 중" 배너). 완료 미열람 수는 내 회의록 네비 배지(`doneUnviewed` Set)
- 상세·편집(detailView)은 리스트/도크/토스트/딥링크에서 진입. 생성 완료 시 결과를 바로 띄우지 않음 — unread로 쌓임 (`viewed_at` 마킹)
- 프로젝트는 업로드 시 **파일 카드마다** 선택 (공통 옵션 아님). 상세에서 분류 변경 가능 (`PUT /summaries/{id}/project`)
- 완료 시 서버가 자동 이메일 발송 — "수정하러 가기" 버튼이 `#summary/{id}` 해시 딥링크로 상세 직행 (`APP_BASE_URL` env)
